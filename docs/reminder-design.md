...
### Phase 3 ―― 自治体の収集日プリセット（★東京23区を着手）
- 「区・地区（丁目）を選ぶと曜日が自動入力」。実装済み（reminder.html の①）。
- **データ源は各区の公式オープンデータCSV**。推測は入れず公式値のみ変換（方針準拠）。
- 生成：`scripts/build-gomi.js`（CSV取得→解析→`data/gomi/<区コード>.json`）。区ごとにフォーマットが違う（CSV／PDF／住所検索式）ため、区ごとにアダプタを追加していく。
- 進捗：**品川区（13109）・港区（13103）内蔵済み**。
  - 品川区：公式オープンデータCSV → 137地区（燃やすごみ/陶器ガラス金属/資源）。`scripts/build-gomi.js`。
  - 港区：CSV無し。公式PDFカレンダー（令和8年度版）が128リンク＝実質15スケジュール（複数地区で共有）。pdfjsで座標解析し「セルの月ブロック＋日付→実際の曜日」で抽出（`scripts/lib-minato-pdf.mjs`＋`build-gomi-minato.mjs`、対応表`minato-map.json`）。64地区。**台場1・2丁目は可燃ごみ収集が無い（管路収集）／資源が瓶缶・P紙に分かれる**特殊ケースも個別対応。目視2スケジュールで検算→全64地区で.ics生成を検証。
- 残り21区：クリーンなCSVがある区（文京区など）はCSV方式、PDF式の区はpdfjs方式で順次。住所検索式のみの区は解析コスト高め。未対応区は公式検索リンクで誘導（既存の「内蔵＋全国リンク」方針と同じハイブリッド）。
- 学び：区ごとにデータ形式が全く違う（品川=CSV／港区=PDF）。年度版に注意（_2025-26は令和7＝旧、現行は令和8年度版）。繁華街・台場は収集頻度が特殊（可燃週3・不燃毎週等）。
- 注意：祝日・年末年始・臨時変更は未反映（データに含まれないため）。UIにも明記済み。
