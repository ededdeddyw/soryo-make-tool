...
- **reminder.html** … ごみ収集日リマインド。**東京23区は区・地区（丁目）を選ぶと公式データから収集日を自動入力**（内蔵8区：品川・港・文京・江東・中央・中野・台東・大田、他区は順次／未対応は公式リンク誘導）。手動入力も可。通知付き`.ics`生成＋Googleカレンダー追加。設定は端末内（localStorage）保存・送信なし。
- **data/gomi/＜区コード＞.json** … 区ごとの収集日データ（13109品川・13103港・13105文京・13108江東・13102中央・13114中野・13106台東・13111大田）。形式は区ごとに異なり、build-gomi.js（CSV縦/横・XLSX）と build-gomi-minato.mjs（PDF）で生成。reminder.html が選択時に読み込む。
