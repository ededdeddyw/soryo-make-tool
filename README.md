...
- **reminder.html** … ごみ収集日リマインド。**東京23区は区・地区（丁目）を選ぶと公式データから収集日を自動入力**（内蔵：品川・港・文京・江東・中央・中野の6区、他区は順次／未対応は公式リンク誘導）。手動入力も可。通知付き`.ics`生成＋Googleカレンダー追加。設定は端末内（localStorage）保存・送信なし。
- **data/gomi/＜区コード＞.json** … 区ごとの収集日データ（13109品川・13103港・13105文京・13108江東・13102中央・13114中野。公式データから生成）。reminder.html が選択時に読み込む。
